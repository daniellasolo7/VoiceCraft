...
class HebrewTextTokenizer:
    """Tokenizer for Punctuated Hebrew (Niqqud)."""

    def __init__(self, separator_word="_"):
        self.separator_word = separator_word

    def to_list(self, text: str) -> List[str]:
        # Replace actual spaces with the designated word separator (usually '_')
        text = text.replace(" ", self.separator_word)
        
        # Tokenize character by character. 
        # In Hebrew Niqqud, letters and vowel marks are separate Unicode characters.
        tokens = list(text)
    ...
    def __call__(self, text: Union[str, List[str]]) -> List[List[str]]:
        ...
        return [self.to_list(t.strip()) for t in text]

def tokenize_text_heb(tokenizer: HebrewTextTokenizer, text: str) -> List[str]:
    # Utility function to get tokens for a single string
    tokens = tokenizer([text])
    return tokens[0]

# class DictaGraphemeTokenizer:
#     def __init__(self, vocab_file):
#         # טעינת ה-Vocab מקובץ הטקסט
...
#         # פירוק הטקסט לתווים (כולל סימני ניקוד כתווים נפרדים)
#         ids = []
#         for char in text:
